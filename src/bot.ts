...
export class WPlaceBot {
	...
	/** Auto-draw mode: re-trigger draw every 30 min when enabled */
	...
	public draw() {
		...
		return this.widget.run(
			...
			async () => {
				...
				const paintBtn = document.querySelector<HTMLButtonElement>('.btn.btn-primary');
				if (paintBtn && !paintBtn.disabled) paintBtn.click();
				this.widget.update();
			},
			() => {
				...
				this.widget.setDisabled('draw', false);
				this.scheduleNextAutoDraw();
			}
		);
	}

	/** Schedule next auto-draw if enabled */
	public scheduleNextAutoDraw() {
		if (this._autoDrawTimer) { clearTimeout(this._autoDrawTimer); this._autoDrawTimer = null; }
		if (!this.autoDraw) return;
		const charges = this.me?.charges?.max ?? 1;
		const SECONDS_PER_CHARGE = 30;
		const EXTRA_SECONDS = 15;
		const totalSeconds = Math.max(60, charges * SECONDS_PER_CHARGE + EXTRA_SECONDS);
		const minutes = Math.round(totalSeconds / 60);
		const ms = totalSeconds * 1000;
		this.widget.status = `Next auto-draw in ${minutes}min (${charges} charges)`;
		this._autoDrawTimer = setTimeout(() => {
			this._autoDrawTimer = null;
			if (!this.autoDraw) return;
			this.draw();
		}, ms);
	}
	...
	public toggleAutoDraw(enabled: boolean) {
